...
def sample_device(rng: np.random.Generator) -> str:
    return rng.choice(["desktop", "mobile", "tablet"], p=np.array([0.333, 0.334, 0.333]))
...
def generate_normal_events(rng: np.random.Generator, customer_profiles: Dict[str, Dict[str, object]], merchant_ids: List[str], target_count: int) -> List[Dict[str, object]]:
    ...
    for _ in range(target_count):
        ...
        device_id = profile["preferred_device"] if rng.random() < 0.8 else sample_device(rng)
        ip_id = f"ip_{rng.integers(1, 22000):06d}"
        shipping = f"ship_{rng.integers(1, 400000):08d}"
        billing = f"bill_{rng.integers(1, 400000):08d}"
...
def generate_coordinated_fraud_events(rng: np.random.Generator, customer_profiles: Dict[str, Dict[str, object]], merchant_ids: List[str], target_count: int) -> List[Dict[str, object]]:
    events: List[Dict[str, object]] = []
    ring_count = target_count // 10
    for _ in range(ring_count):
        ring_customers = [rng.choice(list(customer_profiles.keys())) for _ in range(rng.integers(8, 18))]
        merchant_id = rng.choice(merchant_ids)
        device_id = f"device_grp_{rng.integers(1, 500):04d}"
        ip_id = f"ip_grp_{rng.integers(1, 500):04d}"
        seed_time = sample_timestamp(rng, datetime(2023, 3, 1), datetime(2024, 10, 31))
        for idx in range(10):
            customer_id = ring_customers[idx % len(ring_customers)]
            profile = customer_profiles[customer_id]
            txn_time = seed_time + timedelta(minutes=idx * 15 + int(rng.integers(0, 12)))
            ...
            events.append(
                generate_event(
                    ...
                    device_id=device_id,
                    ip_id=ip_id,
                    shipping_address_id=f"ship_{rng.integers(1, 400000):08d}",
                    billing_address_id=f"bill_{rng.integers(1, 400000):08d}",
                    authentication_status=auth,
                    is_fraud=1,
...
def generate_suspicious_merchant_events(rng: np.random.Generator, customer_profiles: Dict[str, Dict[str, object]], merchant_ids: List[str], target_count: int) -> List[Dict[str, object]]:
    ...
    for _ in range(target_count):
        ...
        device_id = profile["preferred_device"] if rng.random() < 0.5 else sample_device(rng)
        ip_id = f"ip_{rng.integers(1, 22000):06d}"
        shipping = f"ship_{rng.integers(1, 400000):08d}"
        billing = f"bill_{rng.integers(1, 400000):08d}"
        auth = sample_authentication_status(rng, suspicious=True)
...
def add_history_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # ordered["ip_unique_customers_before"] = 0
    # ordered["ip_unique_merchants_before"] = 0
    # for ip_id, g in ordered.groupby("ip_id", sort=True):
    #     ip_rows = g.sort_values(["timestamp", "_history_original_index"], kind="mergesort")
    #     seen_customers = set()
    #     seen_merchants = set()
    #     for ts, ts_group in ip_rows.groupby("timestamp", sort=True):
    #         idx = ts_group.index
    #         ordered.loc[idx, "ip_unique_customers_before"] = len(seen_customers)
    #         ordered.loc[idx, "ip_unique_merchants_before"] = len(seen_merchants)
    #         seen_customers.update(ts_group["customer_id"].unique())
    #         seen_merchants.update(ts_group["merchant_id"].unique())
    ordered["ip_unique_customers_before"] = 0
    ordered["ip_unique_merchants_before"] = 0
    ...
    # ordered is already sorted by timestamp and original index.
    # Process one timestamp at a time so transactions sharing the same
    # timestamp cannot see each other as historical activity.
    for ts, ts_group in ordered.groupby("timestamp", sort=False):
        for idx, row in ts_group.iterrows():
            ip_id = row["ip_id"]

            ordered.at[idx, "ip_unique_customers_before"] = len(
                ip_seen_customers.get(ip_id, set())
            )
            ordered.at[idx, "ip_unique_merchants_before"] = len(
                ip_seen_merchants.get(ip_id, set())
            )

        # Only after ALL rows at this timestamp have been assigned their
        # historical values do we add them to the seen sets.
        for ip_id, ip_group in ts_group.groupby("ip_id", sort=False):
            if ip_id not in ip_seen_customers:
                ip_seen_customers[ip_id] = set()
                ip_seen_merchants[ip_id] = set()

            ip_seen_customers[ip_id].update(ip_group["customer_id"].unique())
            ip_seen_merchants[ip_id].update(ip_group["merchant_id"].unique())
...
def validate_dataset(df: pd.DataFrame) -> Dict[str, object]:
    ...
    recomputed = add_history_features(df.copy())
    columns = [
        "customer_transaction_count_before",
        "customer_avg_amount_before",
        "customer_max_amount_before",
    ...
    for col in columns:
        ...
        recomputed_col = pd.to_numeric(recomputed[col], errors="coerce")
        if col == "hours_since_customer_previous_transaction":
            nan_matches = original.isna() & recomputed_col.isna()
            finite_matches = (~original.isna()) & (~recomputed_col.isna()) & np.isclose(
                original.to_numpy(dtype=float),
                recomputed_col.to_numpy(dtype=float),
                rtol=1e-9,
                atol=1e-9,
                equal_nan=True,
            )
            assert (nan_matches | finite_matches).all(), f"History leakage mismatch in {col}"
        else:
            assert original.reset_index(drop=True).equals(recomputed_col.reset_index(drop=True)), f"History leakage mismatch in {col}"
